...
<c:set var="eccCompletionList" value="${dataMap.eccCompletionList }" />

<link rel="stylesheet" href="<%=request.getContextPath() %>/resources/css/approval/approvalRequest.css">

<%session.setAttribute("path", request.getContextPath() + "/approval/adbox/completion"); %>
<!DOCTYPE html>
	  	...
	  	<div class="container-fluid">
	  		<div class="row md-2">
	  			<div class="col-sm-6">
	  				...
	  				<p class="menu-header this">결재완료함</p>  				
	  			</div>
	  		</div>
	  	</div>
	</section>
		...
		<div class="gw-card">
				...
				<div id="keyword" class="card-tools" style="width: 540px;">	
					...
					<input type="hidden" name="perPageNum" id="perPageNum" value="10">
						...
						<span class="input-group-append">
							<button class="btn btn-basic" type="button" id="searchBtn" data-card-widget="search" onclick="list_go5(1,'completion');">
			...
			<div class="card-body">
				<table class="table text-center">
					...
					<c:if test="${empty eccCompletionList }" >
						<tr>
							<td colspan="7">
							<br/><br/><br/><br/><br/>
					...
					<c:forEach items="${eccCompletionList }" var="completionList">
						<tr style='font-size:0.85em;'>
							<td>
							${fn:substring(completionList.sanctn_no,4,1000000000) }
							</td>
							<td >
							 <span class="col-sm-12 ">${completionList.form_nm }</span>
							</td>
							<td id="title" onclick="javascript:OpenWindow('<%=request.getContextPath()%>/approval/ddbox/detail?sanctn_no=${completionList.sanctn_no }&from=list','상세보기',900,890);">
							${completionList.sj }
							</td>
							<c:choose>
							  <c:when test="${completionList.ipcr_code eq '낮음'}">
								<td>${completionList.ipcr_code}</td>	
							  </c:when>
							  <c:when test="${completionList.ipcr_code eq '보통'}">
								<td style="color: blue;">${completionList.ipcr_code}</td>	
							  </c:when>
							  <c:otherwise>
								<td style="color: tomato;">${completionList.ipcr_code}</td>		
							  </c:otherwise>
							</c:choose>
							<td>
							<fmt:formatDate value="${completionList.rgsde }" pattern="yyyy-MM-dd"/> 
							</td>
							<td>
							<fmt:formatDate value="${completionList.sanctn_tmlmt }" pattern="yyyy-MM-dd"/> 
							</td>
						</tr>
					</c:forEach>	
...
function list_go5(page,url){
	if(!url) url="approval/adbox/completion.do";
