  	...
  	<div>
		<div class="form-box">
			<div class="form-box-header">
				...
				<button type="button" class="form-btn basic-btn" onclick="sanctn_go('G004');">임시저장</button>
				<button type="button" class="form-btn request-btn" onclick="sanctn_go('G001');">결재요청</button>
			</div>
...
<!-- 			          		    <td></td> -->
				      ...
				      <table class="form-table3">
				        ...
				        <tr>
				          ...
				          <td id="prsrv">${erf.prsrv_pd }</td>
				          <td class="form-table3-th">문서번호</td>
				          <td></td>
				          <td class="form-table3-th">보안수준</td>
				          <td id="scrty">${erf.scrty_grad }</td>
				        </tr>
				        <tr>
				        	<td class="form-table3-th">결재기한</td>
				        	<td colspan="2"><input type="date" id="tmlmt"/></td>
				        	<td class="form-table3-th">중요도</td>
				        	<td colspan="2">
						        <select name="ipcr" id="ipcr">
									<option value="H001">낮 음</option>
									<option value="H002">보 통</option>
									<option value="H003">높 음</option>
									<option value="H004">긴 급</option>
								</select>	
				        	</td>
				        </tr>
				        <tr>
				      ...
				      <div id="cngo">
				  	  ...
				  	  <form id="eccstSaveForm" enctype="multipart/form-data" method="POST" action="regist.do">
					      ...
					      <input type="hidden" name="formNo" id="formNo" value="${erf.form_no }" />
...
<!--   	<input type = "hidden" name="saveSanc" value=""/> -->
  	...
  	<input type = "hidden" name="erfID" value="${erf.form_no }"/>
...
// 	    	  console.log(userCheck);
	      ...
	      if ($('input[name="uplFile"]').length != 0) {
	        ...
	        target.addEventListener('change', function(){
	            ...
	            target2 = document.getElementById('Files');
	            target2.innerHTML = fileList;
	        });
	      }
	      
	  	//이의신청
	  	<c:if test="${not empty rgsde}">
	  		var rgsde = "<c:out value='${rgsde}'/>";
	  		$('input[name="startDate"]').val(rgsde);
			$('#form-box-reason').text(rgsde+'일자 근태 이의신청합니다.');
	  	</c:if>
	  	<c:if test="${not empty workTime}">
	  		var workTime = "<c:out value='${workTime}'/>";
	  		$('input[name="endDate"]').val(workTime);
	  	</c:if>
	     
	  	<c:if test="${not empty sDe && not empty eDe}">
	  		var sde = "<c:out value='${sDe}'/>";
	  		$('input[name="startDate"]').val(sde);
	  		var ede = "<c:out value='${eDe}'/>";
	  		$('input[name="endDate"]').val(ede);
	  		$('.vacLength').text(sde+"~"+ede);
			var timetime= new Date(Date.parse(ede)-Date.parse(sde));
// 	  		console.log(timetime.getDate());
	  		$('.vacSum').text(timetime.getDate()+"일");
	  		$('#form-box-reason').text(sde+"부터 "+ede+"까지 휴가신청합니다.")
	  	</c:if>
	     
	  	<c:if test="${not empty rVac}">
	  		var rv = "<c:out value='${rVac}'/>";
			$('.Yearvac').text(rv);
	  	</c:if>
