...
<c:set var="eccCompletionList" value="${dataMap.eccWaitList }" />


<%
session.setAttribute("path", request.getContextPath() + "/approval/adbox/wait");
%>
...
<body>
	  	...
	  	<div class="container-fluid">
	  		<div class="row md-2">
	  			<div class="col-sm-6">
	  				...
	  				<p class="menu-header this">결재대기함</p>  				
	  			</div>
				...
				<div id="keyword" class="card-tools" style="width: 540px;">	
					...
					<input type="hidden" name="perPageNum" id="perPageNum" value="10">
						...
						<span class="input-group-append">
							<button class="btn btn-basic" type="button" id="searchBtn" data-card-widget="search" onclick="list_go5(1,'wait');">
	...
	<div class="card-body">
				<table class="table text-center">
					...
					<c:forEach items="${eccCompletionList }" var="waitList">
						<tr style='font-size:0.85em;'>
							<td>
							${fn:substring(waitList.sanctn_no,4,1000000000) }
							</td>
							<td >
							 <span class="col-sm-12 ">${waitList.form_nm }</span>
							</td>
							<td id="title" onclick="javascript:OpenWindow('<%=request.getContextPath()%>/approval/ddbox/detail?sanctn_no=${waitList.sanctn_no }&from=list','상세보기',900,890);">
							${waitList.sj }
							</td>
							<td>
							${waitList.nm }
							...
							<c:choose>
							  <c:when test="${waitList.ipcr_code eq '낮음'}">
								<td>${waitList.ipcr_code}</td>	
							  </c:when>
							  <c:when test="${waitList.ipcr_code eq '보통'}">
								<td style="color: blue;">${waitList.ipcr_code}</td>	
							  </c:when>
							  <c:otherwise>
								<td style="color: tomato;">${waitList.ipcr_code}</td>		
							  </c:otherwise>
							</c:choose>
							<td>
						<fmt:formatDate value="${waitList.rgsde }" pattern="yyyy-MM-dd"/> 
							</td>
							<td>
...
function list_go5(page,url){
	if(!url) url="approval/adbox/wait.do";
